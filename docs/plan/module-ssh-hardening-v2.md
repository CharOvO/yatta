...
模块保持高风险、默认关闭。所有可能影响远程连接的变更必须先进入执行计划，并在 apply 阶段通过 `sshd -t` 和有效配置校验后才允许 reload SSH 服务。
...
- `modules/ssh-hardening/prompts.sh` 负责读取当前 SSH 配置、确认目标用户和密钥证据、询问策略并登记计划。
- `modules/ssh-hardening/apply.sh` 负责根据 prompt 结果生成 Yatta 管理的 sshd drop-in，并调用 adapter 完成写入、校验和 reload。
...
7. apply 阶段写入 `/etc/ssh/sshd_config.d/00-yatta-hardening.conf`。
8. 执行 `sshd -t`；失败时回滚，不 reload。
9. 使用 `sshd -T` 校验目标值是否生效；失败时回滚，不 reload。
10. 校验通过后 reload SSH 服务，不 restart。

## 实现步骤

- 新增 `ssh-hardening` 模块元数据，标记为 `remote-access`、高风险、运行时默认关闭，并声明早于 `ufw`、晚于 `user`。
- 新增 prompt 流程，覆盖端口、目标用户、密钥证据和基础加固策略。
- 新增 apply 流程，生成独立 drop-in，并把校验、回滚和 reload 委托给 adapter。
