...
- `0-99`：入口后的前置检查与环境摘要，例如 `system-check`。
- `100-199`：本机基础设置，例如 hostname、timezone、非 root 用户。
- `200-299`：基础软件包与包管理准备。
- `300-599`：后续服务类模块预留，例如 Docker、应用运行时。
- `600-799`：远程访问和安全加固预留，例如未来 SSH 加固模块。
- `900-999`：防火墙和最终网络收尾。`ufw` 必须位于此区间，避免早于 SSH 等远程访问模块执行。
...
当用户确认安装且存在缺失包时，安装前必须通过 apt adapter 更新必要状态，避免模块直接散落 apt 命令。packages 模块不得安装 `ufw`，防火墙工具由 `ufw` 模块自行负责。
