...
## 13. 默认模块整改方案（草案，未商讨，仅有参考性模块列表）

本节只记录后续模块重置、优化、添加时的参考方向，不代表已经确认进入默认 v1 流程。
...
- 是否开启 UFW 防火墙，并提示该操作将在 apply 阶段放行 SSH 端口后执行。
- 提示默认执行 `default deny incoming` 和 `default allow outgoing`。
- 检测 SSH 端口，手动确认并再次警告。
- 是否开启 80/443。
- 是否开启其他业务端口。
...
- 常用软件安装：Docker、Node.js、Python、Go、Nginx 等。
- timeshift。
- 安全巡检：Rootkit 检测、异常进程检测、异常端口检测、Cron 检查、SSH 登录分析、Docker 安全检查、生成安全报告。
